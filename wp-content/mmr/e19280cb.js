...
jQuery(document).ready(function(_$){
	...
	if(location.hostname=='wp2.me') {
		var tool=`<div class="w2h-web-tool">
			<a class="w2h_button" id="w2h_build_current" href="#">Build</a>
			<a class="w2h_button" id="w2h_sync_theme" href="#">Theme</a>
		</div>`;
		$(tool).appendTo('body');
	}
	...
	$('body').on('click', '#w2h_sync_theme', function(e){
		e.preventDefault();
		if(!confirm("Sync theme css,js.. are you sure?")) return;
		show_cover();
		$.ajax({
			url: w2h.ajax_url+'?action=w2h_sync_theme',
			type: 'POST',
			dataType:'json',
			method: 'POST',
			data: { action: 'w2h_sync_theme'},
			success: function(r) {
				console.log(r);
				show_cover(0);
			},
