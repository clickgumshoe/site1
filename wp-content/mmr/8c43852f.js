...
;
}catch(e){console.log(e)}/*[w2h-js]*/
try{if(typeof w2h!='object') var w2h={};
var __data={};
...
function getVal(k,v) {
	return typeof __data[k]!='undefined'? __data[k]: v;
}
function delVal(k) {
	delete __data[k];
}

function show_cover(st) {
	var e=$('<div id="w2h_loading" style=""></div>');
	if($('#w2h_loading').length==0) $('body').append(e);else e=$('#w2h_loading');
	if(typeof st=='undefined' || st) e.show();else e.hide();
}
...
<style>
#w2h_loading{
	display: block;position: fixed;top: 0;left: 0;z-index: 100;width: 100vw;height: 100vh;
	background-color: rgba(192, 192, 192, 0.5);
	background-image: url("${w2h.plugin_url}/asset/loading.gif");
...
function find_images() {
	var l, list=[], saved=getVal('saved_images',[]);
	$('img[src*="/wp-content/"],img[data-src*="/wp-content/"]').each(function(j,e){
		l=$(e).attr('src');if(!l)l=$(e).attr('data-src');
		for(var i=0;i<saved.length;i++) {
			if(l.indexOf(saved[i])!=-1) return;	//ignore exist
		}
		if(l.indexOf('/wp-content/')!=-1) {
			list.push(l);
			$(e).addClass('w2s-wrong-dom');
		}
	});
	return list;
}

function fetch_exist_images() {
	return new Promise((resolve, reject)=>{
		$.ajax({
			url: w2h.ajax_url+'?action=w2h_list_savedimg',
			success: function(r){
				resolve(r.files);
			}
		});
...
var _tm1;
jQuery(window).scroll(async function(){
	if(_tm1) clearTimeout(_tm1);
	if(!getVal('saved_images')) return;
	_tm1 = setTimeout(find_images,1000);
	
});

jQuery(document).ready(async function(_$){
	if(typeof $ =='undefined') $=_$;

	$('a[href]:not([href*="html"])').each((i,e)=>{
		var l=e.getAttribute('href');
		if(w2h.home==l.replace(/\/$/g,'')) return;
		if(!l.startsWith('#') && l.indexOf(location.hostname)!==-1) {
			if(w2h.test) $(e).addClass('w2h-invalid-link');	//test
			else $(e).remove();
		}
	});

	if(location.hostname=='wp2.me') {
		var tool=`<div class="w2h-web-tool">
			<a class="w2h_button" id="w2h_build_current" title="Build this post" href="#">Build</a>
			<a class="w2h_button" style="display:none" id="w2h_deploy_all" title="Deploy all" href="#">ReplaceAll</a>
		</div>`;
		$(tool).appendTo('body');
	}
	var currentImage,btnIMGSave = $('<a class="w2h_button w2h_img_save" style="position:absolute;z-index: 10000;font-size:12px" href="#">Save</a>');
	btnIMGSave.hide().appendTo('body');
	setVal('saved_images', await fetch_exist_images());
	find_images();

	$('img').on('mouseover', function(e){
		var cls = e.target.getAttribute('class'), pos;currentImage=e.target;
		if(cls.indexOf('w2s-wrong-dom')==-1) return;
		pos = {top: $(e.target).offset().top, left:$(e.target).offset().left};
		//pos.top=e.target.offsetTop;pos.left=e.target.offsetLeft;
		btnIMGSave.show();
		btnIMGSave.css(pos);
		//console.log(`show save button`,pos,e.target);
	});

	$('body').on('click', '.w2h_img_save', function(e){
		e.preventDefault();
		if(!confirm('Save this image?')) return;
		var l=$(currentImage).attr('src');
		if(!l)return;
		show_cover();
		$.ajax({
			url: w2h.ajax_url+'?action=w2h_save_img',
			type: 'POST',method: 'POST',dataType:'json',
			data: {image_url: l,action: 'w2h_save_img'},
			success: async function(r) {
				console.log(r);
				show_cover(0);
				$(currentImage).removeClass('w2s-wrong-dom');
				setVal('saved_images', await fetch_exist_images());
			},
			error: function(){console.log(`failed ajax`)}
		});
		//console.log(l);
	});

	$('body').on('click', '#w2h_build_current', function(e){
		e.preventDefault();
		if(!confirm("Build this page, are you sure?")) return;
		show_cover();
		$.ajax({
			url: w2h.ajax_url+'?action=w2h_build_post',
			type: 'POST',
			dataType:'json',
			method: 'POST',
			data: {url: location.href.split('?')[0], action: 'w2h_build_post'},
			success: function(r) {
				console.log(r);
				show_cover(0);
			},
	...
	$('body').on('click', '#w2h_deploy_all', function(e){
		e.preventDefault();
		if(!confirm("Sync theme css,js.. are you sure?")) return;
		show_cover();
		$.ajax({
			url: w2h.ajax_url+'?action=w2h_deploy_all',
			type: 'POST',
			dataType:'json',
			method: 'POST',
			data: { action: 'w2h_deploy_all'},
			success: function(r) {
				console.log(r);
				show_cover(0);
			},
...
;
}catch(e){console.log(e)}/*[comment-reply]*/
try{/**
...
;
}catch(e){console.log(e)}/*[flatsome-instant-page]*/
try{// modified to work without type="module"
